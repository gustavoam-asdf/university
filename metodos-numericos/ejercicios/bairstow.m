...
while true%Este ciclo nos permite validar que el primer coeficiente sea 1
  ...
  if fx(1)==1
    ...
    disp('ERROR!!!!!')
    disp('El primer coeficiente debe ser igual a 1, por favor intentede nuevo.')
  end
end
...
%ErrorEr=abs((r1/r)*100);
 
Es=abs((s1/s)*100);
 
if ((abs (Er))&(abs(Es))<=Tol)%Cuando llegamos al error toleradose calculan las raices en esa iteración
...
if (n-1)==2%Cuando la ecuación es de segundo grado se resuelve con elmétodo de fórmula general
...
elseif (n-1)==1%Cuando la ecuación es de primer grado solamente se hacela sustitución.
 
it=it+1;%Se incrementa en 1 la iteración
 
if (abs(fx(2))>=Tol)cRoot=cRoot+1;
